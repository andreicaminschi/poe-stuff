...
| --- | --- | --- |
| `@poe/filter-eval` | `@poe/filter-eval/parse-filter`, `/evaluate-filter`, `/match-filter`, `/filter-ast`, `/format-note` | The `.filter` grammar as code: a parser, an evaluator that decides which block takes an item, and the `#@` note a generated block carries its bucket in. `compileFilter` answers the same question for bulk runs: it compiles the blocks once and indexes them by `BaseType ==`, so a million items cost seconds rather than hours. `compileFilterEvery` does the same but keeps every matching block past the winner. **Depends on nothing, on purpose** — it is the independent reader that checks whatever wrote a filter, and sharing a types file with the writer would end that. See [lib/filter-eval/README.md](lib/filter-eval/README.md). |
| `@poe/filter-compile` | `@poe/filter-compile/compose`, `/fill-from`, `/condition-line`, `/resolve-row`, `/compile-filter`, `/types` | The one copy of how a row's conditions resolve: category, subcategory, item and variant laid over each other, `from: "name"` and `from: "baseTypes"` filled off the row, and a resolved condition written as a `.filter` line from `@poe/filter-eval`'s registry. `compileFilter` writes the unstyled `.filter`, one block per row or variant with its key in the note, with each category's subcategories in their `order` and a `catchAll` one last; `catalog:compile`, `catalog:validate` and the taxonomy's eval cases all use it. `apps/taxonomy` validates with it, so what it reports and what compiles cannot disagree. |
...
| `@poe/filter-validate` | `@poe/filter-validate/samples-of`, `/find-unfiltered`, `/find-fall-through`, `/group-unfiltered`, `/describe-sample`, `/sample-query`, `/report-csv`, `/types` | The filter validator, run by `yarn catalog:validate` and the admin panel's Validate filter. `samplesOf` yields, one at a time, the sample items a subcategory's `samples` sets build for every catalog row, plus its `rejects` laid over each. A top-level category holds no samples. `findUnfiltered` runs them through `@poe/filter-eval`'s `compileFilter` and reports the samples no block takes, grouped by row, plus the paths that have no sets. `findFallThrough` reports samples its own path misses, another path wins, another path also matches, or a reject sample its own path takes; a `catchAll` category may overlap its siblings. What to sample lives in the taxonomy, never here. |
