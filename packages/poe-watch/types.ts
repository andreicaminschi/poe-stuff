 ...
 *
 * `item_id` joins to {@link ItemData}`.id`, though not every id resolves: the two
...
export type ExchangeRatioSide = {
  ...
  /** Currency Exchange pair id, joining to `ExchangePair` on the compact rows. */
 ...
 *
 * `id` joins to {@link ItemData}`.id`. `category` is the same vocabulary as
 * {@link ItemCategory} but arrives as a plain string here — the endpoint publishes no
 * enum, so nothing narrows it.
 */
